...
use super::super::RX_BUFFER_SIZE;
...
impl RDes {
    ...
    /// Configures the reception buffer address and length and passed descriptor
    /// ownership to the DMA
    #[inline(always)]
    fn set_ready(&self, buf: *mut u8) {
        ...
        // Used to flush the store buffer as fast as possible to make the buffer
        // available for the DMA.
    ...
    fn setup(&self, next: Option<&Self>, buf: *mut u8) {
        // Defer this initialization to this function, so we can have `RingEntry` on
        // bss.
...
impl<'a> RDesRing<'a> {
    ...
    pub(crate) fn available(&mut self) -> Option<&mut [u8]> {
        ...
        // Not sure if the contents of the write buffer on the M7 can affects reads, so
        // we are using a DMB here just in case, it also serves as a hint to the
        // compiler that we're syncing the buffer (I think .-.)
        fence(Ordering::SeqCst);

        // We might have to process many packets, in case some have been rx'd but are
        // invalid.
        loop {
