...
use crate::{
    ...
    network::{
        control_message::control_message_space,
...
use super::{
    ...
    interface::sockaddr_storage_to_socket_addr,
};
...
impl NetworkPort for LinuxNetworkPort {
    ...
    async fn send_time_critical(
        ...
        // TODO get a backup send timestamp from somewhere (it must be the same clock used for timestamps!)
...
impl LinuxNetworkPort {
    ...
    fn try_recv_tx_timestamp(
        ...
        // we et back two control messages: one with the timestamp (just like a receive timestamp),
        // and one error message with no error reason. The payload for this second message is kind of
        // undocumented.
        //
        // section 2.1.1 of https://www.kernel.org/doc/Documentation/networking/timestamping.txt says that
        ...
        let (_, control_messages, _) =
            receive_message(tc_socket, &mut [], &mut control_buf, MessageQueue::Error)?;
