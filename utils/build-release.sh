...
package_version=$(cargo read-manifest --manifest-path statime-linux/Cargo.toml | jq -r .version)
...
for target in "${targets[@]}"; do
    dbg_sym_tar="statime_dbg_$package_version-$target.tar.gz"

    echo "--- Calling cross for building statime package for target '$target'"
    cross build --target "$target" --package statime-linux --release --features "${RELEASE_FEATURES:-}"
    ...
    cargo deb --no-build --no-strip --target "$target" --compress-type xz --package statime-linux
